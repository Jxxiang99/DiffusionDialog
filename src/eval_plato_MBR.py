...
class MBR_Evaluator:
    ...
    def eval(self, dataloader, step, split=None, save_result=True, sample_size=10):
        ...
        for i, batch in enumerate(dataloader):
            ...
            bsz = len(gold_temp)
            score_temp = [0 for _ in range(bsz)]
            best_pred_temp = ['' for _ in range(bsz)]
            for j in range(sample_size):
                ...
                for idx in range(bsz):
                    score = bleu_score.sentence_bleu(
                        [gold_temp[idx]], pred_temp[idx], smoothing_function=SmoothingFunction().method7, weights=[1, 0, 0, 0])

                    if score_temp[idx] < score:
                        score_temp[idx] = score
                        best_pred_temp[idx] = pred_temp[idx]
            
            best_pred_sents_all += best_pred_temp
            gold_sents_all += gold_temp
        
        if self.accelerator.is_main_process and save_result:
            self.save_result(gold_sents_all, best_pred_sents_all)

        res = self._compute_metrics(best_pred_sents_all, gold_sents_all)
