...
class DialogCollator:
    def __init__(self, tokenizer, model, max_source_length, max_target_length):
...
class DocDialogCollator:
    ...
    def __call__(self, features):
        ...
        response = []
        for d in features:
            context.append(f"[CONTEXT] {d['context']} [DOCUMENT] {d['document']}")
            response.append(d["response"])
...
class PlanDocDialogCollator:
    ...
    def __call__(self, features):
        ...
        for d in features:
            context.append(f"[CONTEXT] {d['context']} [DOCUMENT] {d['document']}")
            response.append(f"{d['plan']} [RESPONSE] {d['response']}")
...
class ContextDrivenCollator:
    ...
    def __call__(self, features):
        ...
        response = []
        for d in features:
            context.append(f"[CONTEXT] {d['context']}")
            document.append(f"[CONTEXT] {d['context']} [DOCUMENT] {d['document']}")
            response.append(d["response"])
        ...
        encoded_context_document = self.tokenizer(
            document, padding=True, max_length=self.max_source_length, truncation=True, return_tensors="pt"
        )
        model_inputs["doc_input_ids"] = encoded_context_document["input_ids"]
        model_inputs["doc_attention_mask"] = encoded_context_document["attention_mask"]
