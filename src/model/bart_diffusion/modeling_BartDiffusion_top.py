...
class BartDiffusionEncoderOutput(ModelOutput):
    ...
    encoder_latent: torch.FloatTensor = None
...
class BartDiffusionDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartDiffusionConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.model_latent_size = config.model_latent_size
        self.num_head = config.decoder_attention_heads
        self.embed_dim = config.d_model
        ...
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.latent_input_proj1 = nn.Linear(config.d_model * 2, config.decoder_ffn_dim)
        self.latent_input_proj2 = nn.Linear(config.decoder_ffn_dim, config.d_model)
        self.latent_layer = LatentLayer(config)
        self.latent_layernorm = nn.LayerNorm(config.d_model)
        self.final_latent_layer = BartDiffusionLatentLayer(config)
    ...
    def latent_hidden_function(self, hidden_states, decoder_latent):
        #combine the latent with hidden state to fuse in the information of decoder_latent
        residual = hidden_states
        latent_hidden_states = self.latent_layer(
            hidden_states=hidden_states,
            encoder_hidden_states=decoder_latent,
        )
        cat_hidden = torch.cat([latent_hidden_states, hidden_states], dim=-1)
        latent_hidden = self.activation_fn(self.latent_input_proj1(cat_hidden))
        latent_hidden = nn.functional.dropout(latent_hidden, p=self.activation_dropout, training=self.training)
        latent_hidden = self.latent_input_proj2(latent_hidden)
        latent_hidden = nn.functional.dropout(latent_hidden, p=self.dropout, training=self.training)
        hidden_states = residual + latent_hidden
        hidden_states = self.latent_layernorm(hidden_states)
    ...
    def forward(
        ...
        if decoder_latent is None:
            latent_ids = torch.arange(self.latent_size, dtype=torch.long, device=input_ids.device)
            latent_ids = latent_ids[None, :].expand(input_shape[0], -1)
            latent_embeds = self.embed_latents(latent_ids)
            hidden_states = torch.cat([hidden_states, latent_embeds], dim=1)
            input_shape = hidden_states.size()[:-1]
            masks = torch.ones_like(latent_ids, dtype=attention_mask.dtype, device=attention_mask.device)
            latent_attention_mask = _expand_mask(attention_mask, inputs_embeds.dtype, tgt_len=self.latent_size)
            attention_mask = torch.cat([attention_mask, masks], dim=1)
            attention_mask = self._prepare_decoder_attention_mask(
                attention_mask, input_shape, inputs_embeds, past_key_values_length
            )
            attention_mask[:,:,-self.latent_size:,-self.latent_size:] = 0
        else:
        ...
        if decoder_latent is None:
            latent_states = hidden_states[:,-self.latent_size:,:]
            hidden_states = hidden_states[:,:-self.latent_size,:]
            latent_states = self.final_latent_layer(
                hidden_states=latent_states,
                encoder_hidden_states=hidden_states,
                encoder_attention_mask=latent_attention_mask,
            )[0]
            return latent_states, hidden_states
        else:
            hidden_states = self.latent_hidden_function(hidden_states, decoder_latent)
...
class BartDiffusionEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartDiffusionConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.latent_size = config.encoder_latent_size
        self.embed_dim = config.d_model
        ...
        if self.latent_size > 0:
            self.embed_latents = nn.Embedding(config.encoder_latent_size, embed_dim, padding_idx=None)
...
#  + embed_role + embed_turn
        ...
        if self.latent_size > 0:
            latent_ids = torch.arange(self.latent_size, dtype=torch.long, device=hidden_states.device)
            latent_ids = latent_ids[None, :].expand(input_shape[0], -1)
            latent_embeds = self.embed_latents(latent_ids)
            hidden_states = torch.cat([latent_embeds, hidden_states], dim=1)
            masks = torch.ones_like(latent_ids, dtype=attention_mask.dtype, device=attention_mask.device)
...
class BartDiffusionModel(BartPretrainedModel):
    def __init__(self, config: BartDiffusionConfig):
        ...
        self.model_latent_size = config.model_latent_size
        # self.embed_latents = nn.Embedding(config.model_latent_size, config.d_model, padding_idx=None)
        # self.latent_layer = BartDiffusionLatentLayer(config)
        # self.layernorm_embedding = nn.LayerNorm(config.d_model)
        ...
        # self.with_kv_loss = config.loss_dict['with_kv_loss']
        # self.with_diffusion_loss = config.loss_dict['with_diffusion_loss']
        # self.diffusion, self.schedule_sampler = get_gaussian_diffusion_and_sampler(config)
    ...
    # def shrink_latent(self, decoder_latent):
    #     latent_ids = torch.arange(self.model_latent_size, dtype=torch.long, device=decoder_latent.device)
    #     latent_ids = latent_ids[None, :].expand(decoder_latent.shape[0], -1)
    #     latent_embeds = self.embed_latents(latent_ids)
    #     latent_embeds = self.layernorm_embedding(latent_embeds)
    #     latent = self.latent_layer(
    #         hidden_states=latent_embeds,
    #         encoder_hidden_states=decoder_latent,
    #     )
    #     return latent
    ...
    def get_bow_loss(self, latent, labels, decoder_attention_mask):
        ...
        latent_logits = latent_logits.sum(1)
        ...
        bow_loss = bow_loss.sum(-1) / weights
    ...
    def get_sim_loss(self, latent):
        bsz = latent.shape[0]
        latent = latent.view(bsz, -1)
        latent_1 = latent.unsqueeze(0)
        latent_2 = latent.unsqueeze(1)
        sim_loss = ((latent_1 - latent_2) ** 2).mean(-1)
        sim_loss = sim_loss.mean() * (bsz * bsz) / (bsz * bsz - bsz)
        return -1.0 * sim_loss
    ...
    def get_loss_and_logits(
        ...
        decoder_latent, decoder_hidden_state = self.decoder(
        ...
        diffusion_loss = torch.sqrt(diffusion_out["mse"])

        if labels is not None:
            if self.with_diffusion_rc_loss:
                pred_latent = self.get_input_latent(pred_x_start)
                diffusion_hidden_state = self.decoder.latent_hidden_function(hidden_states=decoder_hidden_state, decoder_latent=pred_latent)
            else:
                diffusion_hidden_state = None
            
            if self.with_rc_loss and rc_loss_weight > 0.0:
                gold_latent = self.get_input_latent(decoder_latent)
                hidden_state = self.decoder.latent_hidden_function(hidden_states=decoder_hidden_state, decoder_latent=gold_latent)
            ...
            if self.with_sim_loss:
                ...
                loss['sim_loss'] = sim_loss1
            if self.with_clip_loss:
                encoder_latent = encoder_outputs.encoder_latent
                clip_loss = self.clip(encoder_latent.view(bsz, -1), decoder_latent.view(bsz, -1))
                loss['clip_loss'] = clip_loss
            if self.with_bow_loss:
                loss['bow_loss'] = self.get_bow_loss(decoder_latent, labels)
        else:
            pred_latent = self.get_input_latent(pred_x_start)
            diffusion_hidden_state = self.decoder.latent_hidden_function(hidden_states=decoder_hidden_state, decoder_latent=pred_latent)
            hidden_state = None
    ...
    def get_generation_input(
        ...
        if decoder_input_ids is None:
            ...
            # decoder_latent = self.reverse(decoder_latent)
        else:
            decoder_latent, decoder_hidden_state = self.decoder(
                input_ids=decoder_input_ids,
                attention_mask=decoder_attention_mask,
            ...
            # model_kwargs={'encoder_hidden_state':encoder_outputs.last_hidden_state, 'encoder_attention_mask':attention_mask, 'encoder_latent': encoder_outputs.encoder_latent}
            # cond_fn = None
            # t = torch.tensor([20], device=input_ids.device)
            # t = t.long()
        ...
        decoder_latent = self.get_input_latent(decoder_latent)
...
class BartDiffusionForConditionalGenerationTOP(BartPretrainedModel):
    ...
    def get_loss(
        ...
        loss, hidden_state, diffusion_hidden_states = self.model.get_loss_and_logits(
            input_ids=input_ids,
            role_ids=role_ids,
            turn_ids=turn_ids,
            attention_mask=attention_mask,
        ...
        if labels is not None:
            ...
            if diffusion_hidden_states is not None:
                ...
                # diffusion_weight = 1.5 - t / diffusion.num_timesteps
                if isinstance(schedule_sampler, LossSecondMomentResampler):
                    schedule_sampler.update_with_all_losses(t, diffusion_rc_loss)
            ...
            if hidden_state is not None:
                ...
                rc_loss = rc_loss.sum(-1) / weight * t_weights
